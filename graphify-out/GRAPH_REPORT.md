...
## Corpus Check
- 97 files · ~110,828 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 654 nodes · 1435 edges · 56 communities detected
- Extraction: 47% EXTRACTED · 53% INFERRED · 0% AMBIGUOUS · INFERRED: 766 edges (avg confidence: 0.62)
...
- `RecommendationScanner` --uses--> `Trigger a manual scan. Runs in the background, returns immediately.`  [INFERRED]
  C:\Indian_Swing\indian_swing\recommendations\scanner.py → C:\Indian_Swing\indian_swing\api\routes\scanner.py
- `RecommendationScanner` --uses--> `Trigger a manual scan. Runs in the background, returns immediately.`  [INFERRED]
  C:\Indian_Swing\indian_swing\recommendations\scanner.py → C:\Indian_Swing\indian_swing\api\routes\scanner.py
...
Cohesion: 0.07
Nodes (34): StrategyContext, CLI entry point using Typer. Usage:   swing db init   swing data download --univ, List all discovered strategies., List all discovered strategies., List all discovered strategies., Initialize database tables., Download OHLCV data for the specified universe., Download OHLCV data for the specified universe. (+26 more)

### Community 1 - "Community 1"
Cohesion: 0.06
Nodes (30): BacktestRequest, list_saved_results(), run_backtest(), compute_metrics(), compute_monthly_returns(), _empty_metrics(), _equity_series(), _max_drawdown() (+22 more)
...
Cohesion: 0.07
Nodes (26): BaseRepository, SQLAlchemy sync engine wrapped for async usage via run_in_executor. Python 3.14, Create all tables. Idempotent., Convert aiosqlite URL to sync sqlite URL., Sync context manager for use inside run_in_executor., Sync context manager for use inside run_in_executor., Async-compatible session context manager.     Runs the session synchronously but, Async-compatible session context manager.     Runs the session synchronously but (+18 more)
...
Cohesion: 0.1
Nodes (9): Requested strategy is not registered., StrategyNotFoundError, Auto-discovery strategy registry. Scans the strategies/ package at startup. User, Singleton registry that auto-discovers and holds all BaseStrategy subclasses., Import every module in strategies/ and register concrete BaseStrategy subclasses, StrategyRegistry, ReplayPage(), _serialize_scan() (+1 more)
...
- **Thin community `Community 51`** (1 nodes): `Inject YAML values as env vars if not already set (env vars take priority).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 52`** (1 nodes): `SQLAlchemy ORM models. All tables prefixed with `sw_` to avoid collisions if sha`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 53`** (1 nodes): `Adds standard daily indicators to a daily OHLCV dataframe.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 54`** (1 nodes): `Adds standard weekly indicators to a weekly OHLCV dataframe.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `OHLCVRepository` connect `Community 0` to `Community 1`, `Community 3`, `Community 4`, `Community 5`, `Community 8`?**
  _High betweenness centrality (0.072) - this node is a cross-community bridge._
- **Why does `get_sync_session()` connect `Community 6` to `Community 0`, `Community 2`, `Community 5`, `Community 7`?**
  _High betweenness centrality (0.071) - this node is a cross-community bridge._
- **Why does `Stock` connect `Community 0` to `Community 1`, `Community 3`, `Community 4`, `Community 5`, `Community 7`, `Community 8`?**
