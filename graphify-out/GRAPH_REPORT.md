...
## Corpus Check
- 133 files · ~161,582 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 791 nodes · 1740 edges · 85 communities detected
- Extraction: 50% EXTRACTED · 50% INFERRED · 0% AMBIGUOUS · INFERRED: 872 edges (avg confidence: 0.65)
...
1. `get_sync_session()` - 50 edges
2. `OHLCVRepository` - 46 edges
3. `RecommendationScanner` - 39 edges
4. `Stock` - 38 edges
5. `BaseIndicator` - 30 edges
6. `ScanJob` - 29 edges
7. `Recommendation` - 28 edges
8. `StockRepository` - 26 edges
9. `HistoricalScanManager` - 26 edges
10. `ATR` - 24 edges
...
Cohesion: 0.05
Nodes (41): check_db_rows(), check_jobs(), check_pfc(), check_pfc_history(), check_stock_data(), check_stock_details(), get_sync_session(), find_420() (+33 more)

### Community 1 - "Community 1"
Cohesion: 0.05
Nodes (36): BacktestRequest, list_saved_results(), run_backtest(), BaseStrategy, data_requirements(), IndicatorRequirement, StrategyDataRequirements, StrategySignal (+28 more)

### Community 2 - "Community 2"
Cohesion: 0.06
Nodes (43): Replay Engine — sync DB version., ReplayEngine, ReplaySession, ReplayState, BacktestError, DatabaseError, DataError, DataNotFoundError (+35 more)

### Community 3 - "Community 3"
Cohesion: 0.07
Nodes (27): StrategyContext, IndicatorCalculator, DynamicLookbackEngine, get_required_lookback(), Stock, DataPipeline, _df_to_records(), Build lightweight recommendation dicts and forward to the Telegram notifier. (+19 more)

### Community 4 - "Community 4"
Cohesion: 0.11
Nodes (34): HistoricalScanManager, Evaluate exit triggers for an active trade., Create pending paper trades for all recommendations generated in scan_uuid., Compute final statistics and metrics for completed runs., Validate date format DD/MM/YY and return date object., Generate a professionally formatted Excel workbook representing the trading jour, Find an active historical scan session., Create a new historical scan session. (+26 more)
...
Cohesion: 0.06
Nodes (22): BaseRepository, check(), check(), data_download(), db_init(), list_strategies(), scan_historical(), scan_run() (+14 more)
...
Cohesion: 0.11
Nodes (16): OHLCVCleaner, OHLCVResampler, Data cleaning and corporate action adjustment. All timeframes are derived intern, Clean and adjust raw OHLCV data., Fill gaps in trading days (holidays etc.) using forward fill — max 3 days., Replace negative or zero volume with NaN then ffill., Derive weekly, monthly, quarterly, yearly candles from daily data.     Never cal, DataValidationError (+8 more)

### Community 9 - "Community 9"
Cohesion: 0.23
Nodes (25): BaseModel, DeclarativeBase, Presentation-layer only. Returns universe membership badges for a symbol.     D, Returns only active, ready, or open recommendations from the database., Retrieve full paginated history for audit screens., Saves a recommendation. Prevents duplicates by verifying Symbol, Expiry,     St, Persist market tick. atm_iv stored in payload for future historical IV percentil, A1 — Deterministic synthetic option chain.     Generates LTP values from a simp (+17 more)

### Community 10 - "Community 10"
Cohesion: 0.13
Nodes (16): DataProvider, Provider-agnostic interface for fetching OHLCV data and stock metadata.     Impl, Return True if provider is reachable. Default implementation always True., DataProvider, Exception, ConfigurationError, DataProviderError, Platform misconfiguration detected. (+8 more)

### Community 11 - "Community 11"
Cohesion: 0.13
Nodes (18): add_column(), dispose_engine(), get_engine(), get_session(), get_session_factory(), _get_sync_url(), init_db(), _heal_stale_scan_jobs() (+10 more)
...
Cohesion: 0.18
Nodes (13): add_daily_indicators(), add_relative_strength(), add_weekly_indicators(), build(), IndicatorBundle, _require_length(), validate_required_columns(), check_rules() (+5 more)
...
- **Why does `get_sync_session()` connect `Community 0` to `Community 1`, `Community 3`, `Community 4`, `Community 6`, `Community 7`, `Community 11`, `Community 14`?**
  _High betweenness centrality (0.074) - this node is a cross-community bridge._
- **Why does `OHLCVRepository` connect `Community 4` to `Community 1`, `Community 2`, `Community 3`, `Community 6`, `Community 8`, `Community 9`?**
  _High betweenness centrality (0.071) - this node is a cross-community bridge._
- **Why does `Stock` connect `Community 3` to `Community 1`, `Community 2`, `Community 4`, `Community 7`, `Community 8`, `Community 9`, `Community 11`?**
  _High betweenness centrality (0.053) - this node is a cross-community bridge._
- **Are the 48 inferred relationships involving `get_sync_session()` (e.g. with `check_rules()` and `_heal_stale_scan_jobs()`) actually correct?**
  _`get_sync_session()` has 48 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 37 inferred relationships involving `OHLCVRepository` (e.g. with `Presentation-layer only. Returns universe membership badges for a stock symbol.` and `BacktestConfig`) actually correct?**
  _`OHLCVRepository` has 37 INFERRED edges - model-reasoned connections that need verification._
- **Are the 27 inferred relationships involving `RecommendationScanner` (e.g. with `FastAPI application entry point. Mounts all routers, configures CORS, lifespan` and `Mark any ScanJob still in 'running' status after 2+ hours as 'failed'.     Thes`) actually correct?**
  _`RecommendationScanner` has 27 INFERRED edges - model-reasoned connections that need verification._