# Graph Report - C:\Indian_Swing  (2026-07-23)

## Corpus Check
- 144 files · ~237,046 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 888 nodes · 2146 edges · 91 communities detected
- Extraction: 43% EXTRACTED · 57% INFERRED · 0% AMBIGUOUS · INFERRED: 1220 edges (avg confidence: 0.62)
...
1. `Stock` - 73 edges
2. `ScanJob` - 63 edges
3. `Recommendation` - 60 edges
4. `OHLCVRepository` - 57 edges
5. `get_sync_session()` - 55 edges
6. `StockRepository` - 55 edges
7. `RecommendationScanner` - 50 edges
8. `HistoricalScanSession` - 49 edges
9. `PaperTrade` - 49 edges
10. `HistoricalScanManager` - 48 edges
...
Cohesion: 0.03
Nodes (64): _calculate_metrics(), list_saved_results(), check_db_rows(), check(), check_jobs(), check_pfc(), check_pfc_history(), check() (+56 more)

### Community 1 - "Community 1"
Cohesion: 0.04
Nodes (59): AMRC, data_requirements(), indicator_requirements(), _record(), BacktestRequest, run_backtest(), BaseStrategy, data_requirements() (+51 more)

### Community 2 - "Community 2"
Cohesion: 0.11
Nodes (66): HistoricalScanManager, Evaluate exit triggers for an active trade., Evaluate exit triggers for an active trade., Create pending paper trades for all recommendations generated in scan_uuid., Create pending paper trades for all recommendations generated in scan_uuid., Create pending paper trades for all recommendations generated in scan_uuid., Compute final statistics and metrics for completed runs., Validate date format DD/MM/YY and return date object. (+58 more)

### Community 3 - "Community 3"
Cohesion: 0.04
Nodes (33): ABC, aggregate_monthly(), aggregate_weekly(), DataAggregator, compute(), name(), Base indicator with result caching. All indicators are stateless, vectorized, an, DataFetcher (+25 more)
...
Cohesion: 0.1
Nodes (18): DataProvider, Provider-agnostic interface for fetching OHLCV data and stock metadata.     Impl, Return True if provider is reachable. Default implementation always True., DataProvider, Exception, ConfigurationError, DataProviderError, Platform misconfiguration detected. (+10 more)
...
Cohesion: 0.14
Nodes (16): _detect_vcp(), get_required_lookback(), _df_to_records(), _build_daily_frame(), _seed_stock_and_data(), test_database_integrity_blocks_duplicates_and_broken_references(), test_dynamic_lookback_is_explicit_and_not_source_parsed(), test_strategy_generates_explainable_signal() (+8 more)
...
- **Thin community `Community 79`** (1 nodes): `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 80`** (1 nodes): `Analyzes the quality of a breakout candle, checks the closing position of the pr`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 81`** (1 nodes): `Classifies the current market regime (Bull, Bear, Sideways, Correction, Recovery`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 82`** (1 nodes): `Validates trade and portfolio risk guidelines, checking sector exposure limits`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 83`** (1 nodes): `Computes a unified, fully explainable institutional score (0-100)         based`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 84`** (1 nodes): `Enriches the weekly dataframe with institutional stage classifications,`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 85`** (1 nodes): `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 86`** (1 nodes): `check_aaron.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 87`** (1 nodes): `read_exception.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 88`** (1 nodes): `read_scanner_log.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 89`** (1 nodes): `read_task_log.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `get_sync_session()` connect `Community 0` to `Community 2`, `Community 3`, `Community 6`, `Community 11`, `Community 13`?**
  _High betweenness centrality (0.074) - this node is a cross-community bridge._
- **Why does `Stock` connect `Community 2` to `Community 0`, `Community 1`, `Community 3`, `Community 4`, `Community 6`, `Community 7`, `Community 8`?**
  _High betweenness centrality (0.066) - this node is a cross-community bridge._
- **Why does `OHLCVRepository` connect `Community 2` to `Community 0`, `Community 1`, `Community 4`, `Community 6`, `Community 11`, `Community 14`?**
  _High betweenness centrality (0.063) - this node is a cross-community bridge._
- **Are the 71 inferred relationships involving `Stock` (e.g. with `Get progress of the active historical scan session.` and `List completed historical scans.`) actually correct?**
  _`Stock` has 71 INFERRED edges - model-reasoned connections that need verification._
- **Are the 61 inferred relationships involving `ScanJob` (e.g. with `ProgressPollingFilter` and `FastAPI application entry point. Mounts all routers, configures CORS, lifespan`) actually correct?**
  _`ScanJob` has 61 INFERRED edges - model-reasoned connections that need verification._
- **Are the 58 inferred relationships involving `Recommendation` (e.g. with `Get progress of the active historical scan session.` and `List completed historical scans.`) actually correct?**
  _`Recommendation` has 58 INFERRED edges - model-reasoned connections that need verification._
- **Are the 47 inferred relationships involving `OHLCVRepository` (e.g. with `Presentation-layer only. Returns universe membership badges for a stock symbol.` and `BacktestConfig`) actually correct?**
  _`OHLCVRepository` has 47 INFERRED edges - model-reasoned connections that need verification._